		...
		else if (is_finished == 3)
		{
			...
			while (banker_game == 2 || banker_game == 9)
			{
				...
				while (bustdealer(dealer, count[0]) <= 16)
				{
					...
					cout << "【庄家】 手牌总和为 " << sum << " " << "【庄家】需要继续摸牌" << endl
						 << endl;
					// 给庄家发牌
...
// 玩家的牌库，玩家是否爆掉，玩家个数，每个玩家持有牌数
{
	...
	for (int i = 0; i < player_num; i++)
	{
		...
		if (player_game[i] == 1)
			cout << "【玩家" << i + 1 << "】不能取牌" << endl;
		else if (player_game[i] == -1)
			cout << "【玩家" << i + 1 << "】BOOM!" << endl;
...
{ // 玩家持牌状态，当前玩家在玩家持牌状态中的下标
	cout << "【玩家" << player_index + 1 << "】是否继续发牌?(y/n):";
	string str;
	char ch;
	int temp = 0;
...
void winBlackjack(const Card c[][8], const Card d[][8], int player_game[7], int player_num, int count[8])
{
	...
	else
	{
		if (count_c1 > 21)
		{
			cout << "【庄家】 " << count_c1 << "点,【庄家】BOOM！\n";
			for (int i = 0; i < 7; i++)
			{
				if (player_game[i] == 1)
				{
					cout << "【玩家" << i + 1 << "】" << count_d1[i] << "点,【玩家" << i + 1 << "】WIN!" << endl;
					cout << endl;
				}
			}
		}
		else
		{
			for (int i = 0; i < player_num; i++)
			{
				...
				if (count_d1[i] > 21)
				{
					cout << "【庄家】 " << count_c1 << "点" << endl
						 << "【玩家" << i + 1 << "】" << count_d1[i] << "点	"
						 << "【玩家" << i + 1 << "】BOOM,【庄家】WIN" << endl;
				}
				else
				{
